...
**Chosen: option I ("Fool & Fan")** — picked for phone-home-screen legibility:
the face carries small sizes, the cards provide card-game context. It now lives
as `public/icon.svg` (rendered to `public/icons/*.png` and `src/app/favicon.ico`,
and used as the header logo). The other options are kept here for future
reference.

To roll out a different option later: replace `public/icon.svg`, re-render
`public/icons/*.png` (192/512/1024 `any`, 192/512 `maskable` at 0.92× art scale,
`apple-touch-icon` 180), regenerate `src/app/favicon.ico` (16/32/48), and bump
the service-worker cache version in `public/sw.js` so installed clients pick up
the new artwork.
